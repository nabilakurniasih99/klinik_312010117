...
class Kunjungan extends CI_Controller {
    ...
    function __construct(){
        ...
        $this->load->model('m_kunjungan'); //panggil model
    ...
    function edit($id){
        ...
        $where = array('id_berobat' => $id); //'id' merupakan id dari tabel
        $data['edit'] = $this->m_kunjungan->edit_data($where)->row_array(); //r=pasien
    ...
    //fungsi utk rekam medis
    function rekam($id){ //menangkap id
        $data['title'] = "Rekam Medis";

        //menampilkan detail rekam medis
        $data['d'] = $this->m_kunjungan->tampil_rm($id)->row_array(); //menampilkan data pasien

        //menampilkan riwayat kunjungan
        $q = $this->db->query("SELECT id_pasien FROM berobat WHERE id_berobat='$id'")->row_array();
        
        $id_pasien = $q['id_pasien']; //mendapatkan id pasien
        
        $data['riwayat'] = $this->m_kunjungan->tampil_riwayat($id_pasien)->result_array(); //menampilkan riwayat kunjungan
        ...
        $this->load->view('v_header', $data); //panggil view
        $this->load->view('kunjungan/v_rekam_medis', $data);
        $this->load->view('v_footer');
    }

    function insert_rm(){ //update
        ...
        $this->m_kunjungan->update_data($data, $where); //update
